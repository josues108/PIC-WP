 ...
 * Author: josue
 *
 * Created on January 11, 2026, 5:19 PM
 */


// PIC18F16Q40 Configuration Bit Settings
...
#include <stdio.h>
/*
 Just some notes for now but the xc header file gives me access to all PIC registers and macros

*/

#define _XTAL_FREQ 4000000  // Adjust to your oscillator frequency
#define LED LATBbits.LATB6
#define BUTTON_PORT PORTBbits.RB7

void setup() {
//Doing some basic Pin configuration
    ...
    // Enable IOC on RB7 rising edge
    IOCBPbits.IOCBP7 = 1;     // Positive edge interrupt (button press)
    IOCBNbits.IOCBN7 = 0;    //set as falling edge
    IOCBFbits.IOCBF7 = 0;    // Clear flag
            
    
        // Enable interrupts
    PIE0bits.IOCIE = 1;      // IOC interrupt enable
    INTCON0bits.GIE = 1;     // Global interrupt enable
}
  
void __interrupt(irq(IRQ_IOC), base(0x3008)) IOC_ISR(void) {
    if (IOCBFbits.IOCBF7) {
        LED = 1;
        __delay_ms(1000);
        LED = 0;
        IOCBFbits.IOCBF7 = 0;  // Clear the flag
    }
}
...
void main() {
    setup();
    while (1) {
        // Sleep or idle, or just loop here
    }
}